...
def extract_job_search_params(query: str) -> dict:
    """
    ...
    - page_size: Always set to 10 for initial search
    - work_mode: One of ["work_from_home", "work_from_office"] or omit if not specified
    ...
    4. When specific roles are mentioned (e.g., "data scientist", "software engineer"), prioritize those in the keyword.
    5. Include exact technical skills mentioned (e.g., Python, SQL, React) in both keyword and job_skills parameters.
...
def generate_roadmap(topic: str) -> list:
    """
    ...
    messages = [
        SystemMessage(content=system_prompt),
        HumanMessage(content=f"Create a learning roadmap for: {topic}")
    ]
...
def classify_query(query: str) -> str:
    ...
    if classification not in ["job_search", "roadmap", "normal_text"]:
...
def format_response(query_type: str, query: str, result) -> dict:
    ...
    if query_type == "job_search":
        ...
        if job_count > 0:
            ...
            if location:
                response_text = f"I couldn't find any {role} opportunities in {location} at the moment. Here are some other jobs that might interest you."
            else:
                response_text = f"I couldn't find exact matches for '{role}', but here are some related opportunities you might be interested in."
...
def run_agent(prompt: str) -> dict:
    ...
    if query_type == "job_search":
        # Handle job search
        job_params = extract_job_search_params(prompt)
        return format_response(query_type, prompt, job_params)
    
    elif query_type == "roadmap":
        # Handle roadmap
        roadmap_items = generate_roadmap(prompt)
