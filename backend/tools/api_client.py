...
class GoogleJobsWomenClient:
    ...
    def search_jobs(self, query_params):
        ...
        # Build query string (broad but guided by parameters)
        keyword = query_params.get("keyword", "jobs")
        location = query_params.get("location_name", "India")
        work_mode = query_params.get("work_mode", "")  # Could be remote/onsite
        job_type = query_params.get("job_types", "")
        skills = query_params.get("job_skills", "")

        # Compose a friendly Google Jobs query tailored for women
        query = f"{keyword} jobs for women in {location}"
        if work_mode and "remote" in work_mode:
            query += " remote"
        if job_type:
            query += f" {job_type}"
        if skills:
                ...
                if response.status_code == 200:
                    ...
                    for job in jobs:
                        job_obj = {
                            ...
                            "platform_job_url": job.get("job_highlight_url") or job.get("via", ""),
                            "status": "Active",
                            "posted_on": job.get("extensions", [""])[0] if isinstance(job.get("extensions", []), list) else "",
                        }
                        all_jobs.append(job_obj)
                else:
                    print(f"[ScrapingDog] Failed with {response.status_code}")
...
def get_job_client(platform="herkey"):
    ...
    if platform == "linkedin":
        ...
        return GoogleJobsWomenClient()  # Return an instance of the class, not the class itself
    else:
