    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-white dark:bg-gray-900">
      ...
      >
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`flex mb-6 animate-fade-in ${msg.sender === 'user' ? 'justify-end' : ''}`}
          ...
          >
            ...
            <div className="max-w-[85%] md:max-w-[70%]">
              ...
              {msg.time && <div className="text-xs text-gray-500 mt-1 ml-1">{msg.time}</div>}
            </div>
            {msg.sender === 'user' && (
      ...
      {/* Input area */}
      <div className="p-4 flex items-center gap-2 border-t bg-white dark:bg-gray-900">
        <button onClick={handleFileClick} className="text-gray-500 hover:text-[#9c4d6e]">
          <Paperclip />
        </button>
        <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileChange} />

        <button className="text-gray-500 hover:text-[#9c4d6e]">
          <Plus />
        </button>
        ...
        <input
          ...
          className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-full py-2 px-4 focus:outline-none"
        />

        <button onClick={startListening} className="text-gray-500 hover:text-[#9c4d6e]">
          {isListening ? <Mic className="animate-pulse" /> : <Mic />}
        </button>

        <button onClick={handleSend} className="bg-[#9c4d6e] hover:bg-[#b86a8a] text-white rounded-full p-2 shadow-md">
          <Send />
        </button>
      </div>
    </div>
